...
function AdminDashboard({ onLogout }) {
  ...
  const handleFollowUp = async (projectId, f) => {
    ...
    const res = await fetch(`http://localhost:5000/project/${projectId}/add-report`, {
      method: "POST",
      body: formData,
    });
    const data = await res.json();
  ...
  const deleteClient = async (id) => {
    ...
    const res = await fetch(`http://localhost:5000/admin/delete-client/${id}`, { method: "DELETE" });
    const data = await res.json();
    if (res.ok) {
  ...
  const toggleClient = async (id) => {
    const res = await fetch(`http://localhost:5000/admin/toggle-client/${id}`, { method: "PATCH" });
    const data = await res.json();
    if (res.ok) {
      setClients((prev) =>
        prev.map((c) =>
              ...
              <ul style={{ paddingLeft: "20px" }}>
                {p.reports.map((r) => (
                  <li key={r.id} style={{ marginBottom: "6px", wordBreak: "break-word" }}>
                    ...
                    <a href={`http://localhost:5000${r.download_url}`} target="_blank" rel="noreferrer" style={{ marginLeft: "10px", color: "#0275d8" }}>
                      Download
                    </a>
                    <button
